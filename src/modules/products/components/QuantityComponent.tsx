...
export default function QuantityComponent (props: IQuantityProps) {
  ...
  return (
      <div className="col">
          <button data-testid="removeFromCartBtn"  type="button" className="btn btn-primary btn-sm" onClick={()=>{dispatch(decrementCart(props.itemRecord))}}>-</button>
          <h6 className="quantity-field"> {quantity} </h6>
          <button data-testid="addToCartBtn" type="button" className="btn btn-primary btn-sm" onClick={()=>{ dispatch(addToCart(props.itemRecord))}}>+</button>
